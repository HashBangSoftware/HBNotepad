...
public class ListEdit extends ListActivity
{
    ...
    public boolean onCreateOptionsMenu(Menu menu) 
    {
        ...
        menu.add(0, ADD_ROW_ID, 0, R.string.add_row).setIcon(android.R.drawable.ic_menu_add);
        menu.add(0, REMOVE_ROW_ID, 0, R.string.remove_row).setIcon(android.R.drawable.ic_menu_delete);
        menu.add(1, SAVE_ID, 0, R.string.save_list).setIcon(android.R.drawable.ic_menu_save);
        menu.add(1, SEND_SMS, 0, R.string.send_sms).setIcon(android.R.drawable.ic_menu_send);
        return true;
    }
    ...
    private void removeRow()
    {
    	if(toDoData.size() > 0)
    	{
    		if(mListId == null) //List has not been saved yet (no data persisted)
    		{
    ...
    private void saveList()
    {
        ...
        if (mListId == null)
        {
            ...
            if (id > 0)
            {
                ...
                for(int i = 0; i<toDoData.size(); i++)
                {
                	...
                	long rowId = mDbHelper.createListRowData(mListId, insertRow.getRowDescription(), insertRow.getIsChecked());
        ...
        else
        {
        	 for(int i = 0; i<toDoData.size(); i++)
             {
             	...
             	{
             		mDbHelper.updateListData(insertRow.getId(), insertRow.getRowDescription(), insertRow.getIsChecked());
             	}
             }
    ...
    private void populateList(Bundle savedInstanceState)
    {
        if (mListId != null)
        {
        	if(savedInstanceState == null)
        	{
	            ...
	            contextView.setTitle(lists.getString(lists.getColumnIndexOrThrow(NotesDbAdapter.KEY_TITLE)));
	            ...
	            if (listDataRaw.getCount() > 0)
	            {
	            	do
	            	{
	            		ListRow row = new ListRow(
	            				listDataRaw.getLong(listDataRaw.getColumnIndex("_id")),
	            				listDataRaw.getLong(listDataRaw.getColumnIndex("list_id")),
	            				listDataRaw.getString(listDataRaw.getColumnIndex("item_data")),
	            				listDataRaw.getInt(listDataRaw.getColumnIndex("checked")));
	            		toDoData.add(row);
	...
	{
		ArrayList<Parcelable> savedRows = savedState.getParcelableArrayList(SAVED_DATA);
